...
from utils import load_pretrained_vit_and_add_custom_head, prepare_shapefile_dataset_cont, prepare_shapefile_dataset, remove_black_tiles, correct_duplicate_roi, stack_samples
...
def do_train_cat(
    ...
    for batch in train_dataloader:
        ...
        if do_forward:
            ...
            loss = loss_f(output, torch.tensor(labels).cuda().type(torch.long))
    ...
    print(f"Avg loss Train: {train_loss:>8f} \t Avg loss Validation: {valid_loss:>8f}")
...
def train_loop(
    ...
    for batch in train_dataloader:
        ...
        if do_forward:
            ...
            labels = list(gdf.loc[gdf['bboxes'].isin(bboxes)][target_variable])
            output = model(imgs_tensor.cuda())
            output = output.squeeze() # if extra dimension ?
    ...
    for batch in test_dataloader:
        ...
        if do_forward:
            ...
            with torch.no_grad():
                ...
                list_predictions += list(preds.argmax(1).detach().cpu().numpy())
                list_gt += labels
                list_bbox += bboxes
                loss = loss_f(preds, torch.tensor(labels).cuda().type(torch.long))
                valid_loss += loss.item()
...
def train_classification(
    ...
    print(classification_report(merged_df['observed'], merged_df['predicted'], labels = merged_df['observed'].unique()))
    print(confusion_matrix(merged_df['observed'], merged_df['predicted'], labels = merged_df['observed'].unique()))
...
if __name__ == "__main__":
    ...
    for target_variable in ["cluster_in"]:
